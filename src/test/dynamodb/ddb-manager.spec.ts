...
test('DynamoDBManager.deletePoint calls deleteItem with the correct arguments ', async (t) => {
  let called = false;
...
test('DynamoDBManager.putPoint calls putItem with the correct arguments ', async (t) => {
  let called = false;
  const locx = new GeoTable({
    client: {
